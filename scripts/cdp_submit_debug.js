...
ws.on('message',d=>{const m=JSON.parse(d);
  ...
  else if(m.method==='Network.requestWillBeSent'){ const u=m.params.request.url; if(/recruitify|api|apply|submit|job/i.test(u)) console.log('REQ', m.params.request.method, u.slice(0,90)); }
  else if(m.method==='Network.responseReceived'){ const u=m.params.response.url; if(/recruitify|api|apply|submit|job/i.test(u)) console.log('RESP', m.params.response.status, u.slice(0,90)); }
  else if(m.method==='Runtime.consoleAPICalled'){ const txt=m.params.args.map(a=>a.value!==undefined?a.value:(a.description||'')).join(' '); console.log('CONSOLE', m.params.type, txt.slice(0,200)); }
  else if(m.method==='Runtime.exceptionThrown'){ console.log('EXC', JSON.stringify(m.params.exceptionDetails.exception&&m.params.exceptionDetails.exception.description||m.params.exceptionDetails.text).slice(0,200)); }
});
