  ...
  try{
    ...
    const r=await send('Runtime.evaluate',{expression:`(()=>{
      ...
      for(const a of links){
        ...
        const key=href+'|'+txt.slice(0,40);
        if(seen.has(key)) continue; seen.add(key);
        if(/java|kotlin|spring|backend|develop|engineer|php|laravel|nest|node|senior|mid/i.test(txt) && href.includes('justjoin')){
          out.push({href, txt:txt.slice(0,90)});
        }
