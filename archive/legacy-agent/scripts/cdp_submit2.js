...
ws.on('message',d=>{const m=JSON.parse(d);
  ...
  else if(m.method==='Network.requestWillBeSent'){ const u=m.params.request.url; if(!noise.test(u)) console.log('REQ',m.params.request.method,u.slice(0,120)); }
  else if(m.method==='Network.responseReceived'){ const u=m.params.response.url; if(!noise.test(u)) console.log('RESP',m.params.response.status,u.slice(0,120)); }
});
...
ws.on('open',async()=>{
  try{
    ...
    if(!g.result.value||g.result.value<10){console.log('CAPTCHA NOT SOLVED');ws.close();process.exit(2);}
    // click Apply via JS (reliable, same-origin)
    const clk=await send('Runtime.evaluate',{expression:`(()=>{const b=Array.from(document.querySelectorAll('form#frm button')).find(x=>(x.textContent||'').includes('Apply'))||Array.from(document.querySelectorAll('button')).find(x=>(x.textContent||'').includes('Apply')); if(!b) return 'no-btn'; b.click(); return 'clicked';})()`,returnByValue:true});
    console.log('apply click result:', clk.result.value);
    await sleep(6000);
    const st=await send('Runtime.evaluate',{expression:`(()=>{const t=document.body.innerText.toLowerCase();const succ=['dziękujemy','thank','wysłan','wyslano','została','został','otrzymaliśmy','pomyślnie','submitted','success','aplikacja'];return {gresp:document.querySelector('#g-recaptcha-response').value.length, succ:succ.filter(k=>t.includes(k)), hasForm:!!document.querySelector('form#frm'), hasApply:!!Array.from(document.querySelectorAll('button')).find(b=>(b.textContent||'').includes('Apply')), url:location.href};})()`,returnByValue:true});
