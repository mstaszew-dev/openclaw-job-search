...
ws.on('open',async()=>{
  try{
    ...
    const r=await send('Runtime.evaluate',{expression:`(()=>{
      ...
      const salMatch=t.match(/(\\d[\\d\\s]*\\s?-\\s?\\d[\\d\\s]*\\s?(PLN|zł|EUR|USD|GBP))/i);
      const recMatch=t.match(/(\\d[\\d\\s]+)\\s?(PLN|zł|EUR|USD|GBP)/i);
      return {
        title:document.title,
        salaryRange: salMatch?salMatch[0]:(recMatch?recMatch[0]:null),
        hasInlineApply: !!document.querySelector('#applyButton, button#applyButton'),
        applyBtn: btns.slice(0,3).map(b=>({text:(b.textContent||'').trim().slice(0,30), id:b.id||null})),
        iframeRecaptcha: [...document.querySelectorAll('iframe')].some(f=>(f.src||'').includes('recaptcha')),
        desc: t.slice(0,700)
      };
    })()`,returnByValue:true});
