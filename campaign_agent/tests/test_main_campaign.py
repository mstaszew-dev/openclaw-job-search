...
async def test_gateway_down_connection_errors_abandon_tick_early(tmp_path):
    ...
    try:
        ...
        def fail_conn(llm, tools, messages, max_steps, **kwargs):
            return MagicMock(success=False, reason="llm_error: Connection error.", submitted=0)
        ...
        with patch("campaign_agent.main.run_agent_turn", side_effect=fail_conn) as mock_turn, \
             patch("campaign_agent.main.asyncio.sleep", side_effect=fake_sleep):
            await run_campaign(cfg)
