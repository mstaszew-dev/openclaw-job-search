...
## Job Search Campaign Mission

This workspace is a dedicated job-application agent. Its live campaign state is not copied here:

- `campaign/` is a symlink to `/Users/mst/Downloads/job-search/job-apply`.
- `joblooper/` is a symlink to `/Users/mst/ZCodeProject/joblooper`.
- Treat `campaign/tracker.json` and the campaign scripts as canonical mutable state.

For every explicit `continue` or `run-one-job` request:

1. Read `campaign/AGENT_TICK.md`, `campaign/CONTEXT.md`, and current `campaign/tracker.json`. Run `campaign/tick_status.sh`; never assume the submitted count.
2. Select exactly one mid-to-senior Java/Kotlin/Spring, PHP/Laravel/Symfony, or Node/React/NestJS role (junior/entry-level/graduate roles also allowed per policy widened 2026-07-05, but mid/senior preferred). Reject lead, manager, architect, director, head, and vp roles. Israel may be on-site, hybrid, or remote (onsite/hybrid in central Israel OK; remote OK anywhere). Poland/EU/global must be fully remote; if compensation is listed, require B2B at least 15,000 PLN monthly. Freelance, contract, and part-time engagements are also accepted (in addition to full-time).
3. Dedupe using RAG (`rag_search_apps`) and Gmail search via Playwright (inbox + sent, 60d). Score the candidate yourself using LLM reasoning against your CV knowledge. Do NOT call automation scripts (no score_candidate.py, no check_dupe.py, no preflight scripts). Only `update_tracker.py` is allowed, and only to record a confirmed submission.
